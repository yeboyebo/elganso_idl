...
class eg_notificacionesajustestocks(interna_get):
    ...
    def start(self, params):
        ...
        try:
            if "key" in params and params['key'] == "34762d577d2c6132417e5e5e2f":
                if "notificacion" in params and params['notificacion'] != "":
                    ...
                    sku = params['notificacion']['sku']
                    if not sku or str(sku) == "":
                        res[0] = "KO"
                        if res[1] != "":
                            res[1] += " "
                        res[1] += "El nodo Sku está vacío."
                else:
                    res[0] = "KO"
                    if res[1] != "":
                        res[1] += " "
                    res[1] += "No se ha encontrado el JSON de la notificacion de Ajuste de Stock."
            else:
                res[0] = "KO"
                if res[1] != "":
                    res[1] += " "
        ...
        if notificacion and notificacion != "":
            if not qsatype.FLSqlQuery().execSql("INSERT INTO eg_notificacionesajustestocks (fechanotificacion, horanotificacion, notificacion, sku, estado, procesar) VALUES ('{}', '{}', '{}', '{}', '{}', '{}')".format(str(fechaActual), str(horaActual), cuerpolog, sku, estado, procesar)):
                return {"Mensaje": "ERROR.", "status": 0}

        return {"Mensaje": "OK", "status": 10}

# @class_declaration revoke #
class notificacionesajustestocks(eg_notificacionesajustestocks):
    pass
