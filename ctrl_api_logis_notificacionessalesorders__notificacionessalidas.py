...
class eg_notificacionessalidas(interna_get):
    ...
    def start(self, params):
        ...
        if notificacion and notificacion != "":
            if not qsatype.FLSqlQuery().execSql("INSERT INTO eg_notificacionessalidas (fechanotificacion, horanotificacion, notificacion, sku, estado, procesar) VALUES ('{}', '{}', '{}', '{}', '{}', '{}')".format(str(fechaActual), str(horaActual), cuerpolog, sku, estado, procesar)):
                return {"Mensaje": "ERROR.", "status": 0}

        return {"Mensaje": "OK", "status": 10}

# @class_declaration revoke #
class notificacionessalidas(eg_notificacionessalidas):
    pass
